//
// Created by hujieqing on 2018/7/5.
//

#include "wf_aes_cipher.h"
...
int AES_ECB_Cipher::encode(const unsigned char* src, uint32_t src_len, unsigned char* dest, uint32_t& len)
{
    ...
    if (NULL == src)
    {
        XGL_LOG_WARNING("AES_ECB_Cipher::encode src is NULL ");
        return -1;
    }
    ...
    // PKCS5Padding rules: ฒน(16-len)ธ๖(16-len)
    ...
    {
        XGL_LOG_WARNING("AES_ECB_Cipher::encode, fail to encrypt src");
        return -1;
    }
    dest[len] = 0;
    return 0;
...
int AES_ECB_Cipher::decode(const unsigned char* src, uint32_t src_len, unsigned char* dest, uint32_t& len)
{
    ...
    if (NULL == src)
    {
        XGL_LOG_WARNING("AES_ECB_Cipher::decode src is NULL ");
        return -1;
    }
    // make sure dest buff is larger than src buff
    if(len < src_len)
    ...
    {
        XGL_LOG_WARNING("AES_ECB_Cipher::decode, src len has to be divided by 16");
        return -1;
    }
    ...
    if (len < 0 || len >= src_len)
    {
        XGL_LOG_WARNING("AES_ECB_Cipher::decode, fail to decrypt src");
        return -1;
    }
    dest[len] = 0;
    return 0;
...
void AES_ECB_Cipher::MixColumns()
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        Tm = (*mState)[0][i] ^ (*mState)[1][i] ; Tm = xtime(Tm); (*mState)[0][i] ^= Tm ^ Tmp ;
        Tm = (*mState)[1][i] ^ (*mState)[2][i] ; Tm = xtime(Tm); (*mState)[1][i] ^= Tm ^ Tmp ;
        Tm = (*mState)[2][i] ^ (*mState)[3][i] ; Tm = xtime(Tm); (*mState)[2][i] ^= Tm ^ Tmp ;
        Tm = (*mState)[3][i] ^ t ; Tm = xtime(Tm); (*mState)[3][i] ^= Tm ^ Tmp ;
    }
}
